...
const StaffPortal = {
  ...
  async loadOrders() {
    ...
    const readyOrders = orders.filter(o => o.status.toLowerCase() === 'ready for pickup');
  ...
  renderOrderCards(orders, colType) {
    ...
    return orders.map(o => `
      <div class="staff-order-card">
        ...
        ${o.status.toLowerCase() === 'ready for pickup' ? `<div style="font-size: 0.8rem; background: #064e3b; color: #34d399; padding: 0.3rem 0.5rem; border-radius: 4px; font-weight: 700; text-align: center; margin-bottom: 0.4rem;">Pickup PIN: ${o.pickupPin}</div>` : ''}
